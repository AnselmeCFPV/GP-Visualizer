...
import { distance2D, distance3D, lerpPoint } from './geo';

/** Ferme la boucle en ajoutant le premier point à la fin si nécessaire */
...
/**
 * Chaikin — coupe les angles pour arrondir le polygone.
 * Chaque itération double le nombre de points et adoucit les virages.
 */
...
/**
 * Catmull-Rom centripète (alpha = 0.5) — courbes fluides sans passer
 * brutalement par chaque sommet anguleux du détourage.
 */
...
/** @deprecated Utiliser catmullRomCentripetalClosed */
export function catmullRomClosed(
  controlPoints: LocalPoint[],
  samplesPerSegment = 8,
): LocalPoint[] {
  return catmullRomCentripetalClosed(controlPoints, samplesPerSegment);
}

/** Rééchantillonne une polyligne fermée à pas d'arc régulier (nombre fixe) */
...
/** Rééchantillonne à pas d'arc constant (mètres) pour des virages réguliers */
export function resampleByArcLengthStep(
  points: LocalPoint[],
  stepMeters: number,
): LocalPoint[] {
  const closed = ensureClosed(points);
...
export interface SmoothBorderOptions {
  /** Passes Chaikin (arrondit les angles du détourage) */
  chaikinIterations?: number;
  /** Échantillons Catmull-Rom par segment */
  samplesPerSegment?: number;
  /** Pas d'arc final (mètres) */
  arcStepMeters?: number;
}

/**
 * Pipeline complet de lissage pour une bordure de circuit :
 * Chaikin → Catmull-Rom centripète → rééchantillonnage fin.
 */
...
/**
 * Décale une courbe fermée perpendiculairement.
 * `inward` = vers l'intérieur du polygone (centre du circuit).
 */
