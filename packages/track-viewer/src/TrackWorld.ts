...
const TRACK_MESH_NAME = 'track-mesh';
...
export class TrackWorld implements TrackWorldHandle {
  ...
  private async init(options: TrackWorldOptions): Promise<void> {
    ...
    const trackData = buildTrackScene(options.borders, {
      trackWidth,
      kerbWidth,
      grassRadius: options.grassRadius ?? 120,
      textures,
      localElevationProfile: options.localElevationProfile,
    });
  ...
  getGeoOrigin() {
    const track = this.scene.getObjectByName(TRACK_MESH_NAME);
    return (
