  ...
  updateFromDistance(
    ...
    if (!smooth) {
      if (this.positionSmoothed === null) {
        ...
        this.positionSmoothed.x = _pivot.x;
        this.positionSmoothed.z = _pivot.z;
        const yAlpha = 1 - Math.exp(-dt * 16);
        this.positionSmoothed.y += (_pivot.y - this.positionSmoothed.y) * yAlpha;
      }
